  ...
  const approveMutation = useMutation({
    mutationFn: () =>
      PutApproveRequest(
        project?.id,
        id,
        isRejected ? (request == 0 ? 3 : 1) : request,
      ),
    onSuccess: (data) => {
      // @ts-ignore
    ...
    >
      ...
      <Box>
        <Box>
          ...
          {GetContext("request_to", lang)}{" "}
          <span className="font-bold">{getStatusText(request)}</span>
          &nbsp;{lang == "en" ? "a " : ""}
          <span
            className="font-bold underline text-cyan-600 cursor-pointer"
          ...
          <span className="font-bold">
            {getLocaleValue(project?.name, lang)}
          </span>
        </Box>
...
const RequestLogs = () => {
  ...
  const { data: allProjects = [], refetch: refetchAllProject } = useQuery<
    Project[]
  >({
  ...
  return (
    <Box className="border-1 boxShadow-1 h-full">
      ...
      <Box className="px-[1rem] h-[90vh] overflow-y-scroll">
        {isRequestingLogsError && (
          <div>{GetContext("fail_loaddata", lang)}</div>
        )}
        {isRequestingLogs ? (
          <>
            <LinearProgress />
          ...
          <>
            {requestLogs.length == 0 && (
              <Typography className="text-center mt-4">
                {GetContext("no_request_logs", lang)}
              </Typography>
            )}
