...
const fetchProvince = async (region: string, projectId: string) => {
  ...
  const response = await axios.get(
    `/api/get-calibration-location/${encodedIds}`,
  );
...
const fetchDistrict = async (
  ...
  const encodedIds = encodeURIComponent(
    `${region}/${projectId}/${regionParentId}`,
  );
  const response = await axios.get(
    `/api/get-calibration-location/${encodedIds}`,
  );
...
const Calibration = ({ lang }: { lang: string }) => {
  ...
  const { data: communeData = [], isLoading: isCommuneLoading } = useQuery<
    Commune[]
  >({
  ...
  const { data: villageData = [], isLoading: isVillageLoading } = useQuery<
    Commune[]
  >({
  ...
  const qrcodeMutation = useMutation<unknown, Error, CalibrateProjectBody>({
    ...
    onError: (error: any) => {
      enqueueSnackbar(
        error.response?.data?.error?.message || "Error calibrating project.",
        {
          variant: "error",
          anchorOrigin: {
            vertical: "top",
  ...
  const handleGenerateQrcode = () => {
    if (selectedProject && selectedProvince) {
      ...
      enqueueSnackbar(GetContext("required_field_msg", lang), {
        variant: "warning",
      });
    }
  };
  ...
  return (
    <Grid container spacing={3}>
            ...
            >
              ...
              <FormControl sx={{ minWidth: "100%", marginBottom: 2 }}>
                <InputLabel id="province-filter-label">
                  {GetContext("select_province", lang)}
                </InputLabel>
                <Select
                  variant="standard"
              ...
              {selectedProvince && (
                <FormControl sx={{ minWidth: "100%", marginBottom: 2 }}>
                  <InputLabel id="district-filter-label">
                    {GetContext("select_district", lang)}
                  </InputLabel>
                  <Select
                    variant="standard"
              ...
              {selectedDistrict && (
                <FormControl sx={{ minWidth: "100%", marginBottom: 2 }}>
                  <InputLabel id="commune-filter-label">
                    {GetContext("select_commune", lang)}
                  </InputLabel>
                  <Select
                    variant="standard"
                    ...
                    id="commune-filter"
                    value={
                      selectedCommune ? JSON.stringify(selectedCommune) : ""
                    }
              ...
              {selectedCommune && (
                <FormControl sx={{ minWidth: "100%", marginBottom: 2 }}>
                  <InputLabel id="village-filter-label">
                    {GetContext("select_village", lang)}
                  </InputLabel>
                  <Select
                    variant="standard"
      ...
      >
        {qrCode ? (
          <Image src={qrCode} width="350" height="350" alt="QRCode" />
        ) : (
          <InfoIcon sx={{ fontSize: "12rem" }} />
        )}
        {!(selectedProject && selectedProvince) && (
          <Typography variant="h5">
            {GetContext("calibrate_msg", lang)}
          </Typography>
        )}
      </Grid>
    </Grid>
