...
const fetchProjectDetail = async (projectId: string) => {
  try {
    const response = await axios.get(`/api/config`, {
      params: {
        endpoint: `project/project-details/${projectId}?edit_project=1`,
      },
    });
...
const EditProjectPage: React.FC<EditProjectPageProps> = ({
  ...
  // Project settings
  const [isSurveyLanguageInEnglish, setIsSurveyLanguageInEnglish] =
    useState(true);
    ...
    fetchProjectDetail(projectId).then((data) => {
      ...
      if (data.project.method !== undefined) {
        setDataCollectionMethod({
          method:
            data.project.method === 1
              ? PROJECT_DATA_COLLECTION_METHOD.WEB
    ...
    switch (activeStep) {
      case 0:
        if (projectTitle.en.length < 3 || projectDescription.en.length < 3) {
          showSnackbar(
            "Project title and description must be at least 3 characters long",
            "warning",
          );
          return;
        }
      ...
      case 1:
        if (
          ...
          showSnackbar(
            "Please select at least one location from each region",
            "warning",
          );
          return;
        }
      ...
      case 2:
        ...
        if (
          dataDesignForms.some(
            (form) =>
              form.label.en.length < 0 ||
              dataDesignForms.some((form) => form.type == ""),
          )
        ) {
  ...
  const handleUpdateProject = async () => {
    const selectedProvinceIds = GetLocationIdsFromLocal(
      "selectedProvinces-edit",
    );
    const selectedCommuneIds = GetLocationIdsFromLocal("selectedCommunes-edit");
    const selectedDistrictIds = GetLocationIdsFromLocal(
    ...
    console.log("Update locales: ", localeSetting);
