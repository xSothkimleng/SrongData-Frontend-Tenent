...
const TableActionMenu: React.FC<{
  ...
  const updateProjectUsersMutation = useMutation<unknown, Error, any>({
    mutationFn: async (data: any) => {
      ...
      const encodedIds = encodeURIComponent(`${data.projectId}`);
      const res = await axios.put(`/api/update-project-user/${encodedIds}`, {
        users: userIds,
      });
  ...
  const deleteProjectMutation = useMutation<Error>({
    mutationFn: async () => {
      ...
      const res = await axios.put(`/api/delete-project/${encodedIds}`, {
        password: password,
      });
      // console.log('Update project users:', res.data);
      return res.data;
  ...
  const handleViewProject = (projectId: string) => {
    handleClose();
    router.push(`/dashboard/project-management/project-detail/${projectId}`);
  };
  ...
  const handleViewProjectResponses = () => {
    ...
    handleClose();
    router.push(`/dashboard/project-management/project-responses${query}`);
  };
