...
const CommuneSelectionTab = ({ isUpdate, isEdit }: IsUpdateProps) => {
  ...
  const allCommuneIds = communes
    .map((district) => district.communes.map((commune) => commune.id))
    .flat();
  ...
  const handleSelectCommuneGroup = (
    communes: Location[],
    districtId: string,
    allSelected: boolean,
  ) => {
  ...
  const handleSelectCommune = (communeId: string, districtId: string) => {
    if (selectedCommunes[districtId].selected?.includes(communeId)) {
      ...
      setSelectedCommunes((prev) => ({
        ...
        [districtId]: {
          ...prev[districtId],
          selected: prev[districtId]?.selected
            ? [...prev[districtId].selected, communeId]
            : [communeId],
        },
      }));
    }
  ...
  const CommuneList = React.memo(
    ({
      ...
      const allChecked = selectedCommune
        ? district.communes.length === selectedCommune.selected?.length
        : false;
      const someChecked = selectedCommune
        ? selectedCommune.selected?.length > 0
        : false;
                  ...
                  <ListItemButton sx={{ pl: 4 }} key={commune.id}>
                    <FormControlLabel
                      ...
                      control={
                        <Checkbox
                          disabled={isEdit && isEdit === true}
                          checked={selectedCommune?.selected?.includes(
                            commune.id,
                          )}
                          onChange={() =>
                            handleSelectCommune(commune.id, districtId)
                          }
                        />
                      }
                    />
    ...
    (prev, next) => {
      return (
        ...
        prev.selectedCommune?.selected?.length ==
          next.selectedCommune?.selected?.length
      );
    },
  );
  ...
  const Row = ({ index, style }: { index: number; style: object }) => {
    ...
    return (
      <div style={{ ...style }}>
        <CommuneList
          district={district}
          selectedCommune={selectedCommunes[district.id]}
        />
      </div>
    );
  };
