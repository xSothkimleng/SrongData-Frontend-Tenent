        ...
        <div key={index} className='mb-4 p-3 bg-gray-50 rounded-lg'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
            <div className='p-2 bg-red-50 border border-red-200 rounded'>
              ...
              <pre className='mt-1 text-sm text-gray-700 whitespace-pre-wrap'>{formatValue(item.old)}</pre>
            </div>
            <div className='p-2 bg-green-50 border border-green-200 rounded'>
              <span className='text-xs text-green-600 font-medium'>New Value:</span>
              <pre className='mt-1 text-sm text-gray-700 whitespace-pre-wrap'>{formatValue(item.new)}</pre>
            </div>
          </div>
        </div>
      );
    ...
    return list.map((item, index) => {
      ...
      return (
        <div key={questionId} className='mb-6 p-3 bg-gray-50 rounded-lg border'>
          <h4 className='font-semibold mb-2 text-gray-700'>Question ID: {questionId}</h4>
          {questionLabel && (
            <h4 className='font-semibold mb-2 text-gray-700'>Question Label: {JSON.stringify(questionLabel, null, 2) ?? ''}</h4>
          )}
          ...
          {Object.entries(changes).map(([fieldName, change]: [string, any]) => {
            ...
            return (
              <div key={fieldName} className='mb-4'>
                <h5 className='text-sm font-medium text-gray-600 mb-1 capitalize'>{fieldName.replace(/_/g, ' ')}</h5>
                <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
                  <div className='p-2 bg-red-50 border border-red-200 rounded'>
                    <span className='text-xs text-red-600 font-medium'>Old Value:</span>
                    <pre className='mt-1 text-sm text-gray-700 whitespace-pre-wrap'>{formatValue(oldVal)}</pre>
                  </div>
                  <div className='p-2 bg-green-50 border border-green-200 rounded'>
                    <span className='text-xs text-green-600 font-medium'>New Value:</span>
                    <pre className='mt-1 text-sm text-gray-700 whitespace-pre-wrap'>{formatValue(newVal)}</pre>
                  </div>
                </div>
              </div>
            );
    ...
    Object.entries(metadata).map(([key, value]) => {
      ...
      if (key === 'changed_answers') {
        if (Array.isArray(value)) {
          return (
            <div key={key}>
              <h4 className='text-md font-semibold mb-3 text-gray-800 capitalize'>{key.replace(/_/g, ' ')}</h4>
          ...
          return (
            <div key={key}>
              ...
              {'questions' in value && Array.isArray(value.questions) && (
                <>
                  <p className='text-sm font-semibold mt-2 mb-1 text-gray-700'>Questions</p>
                    ...
                    {Object.entries(value)
                      ...
                      .map(([field, change]) => (
                        <tr key={field}>
                          ...
                          <td className='border p-2'>{formatValue(change.old)}</td>
                          <td className='border p-2'>{formatValue(change.new)}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
        ...
        <div key={key} className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-4'>
          <div>
            ...
            <pre className='text-sm text-gray-800 whitespace-pre-wrap font-mono'>
              {typeof value === 'string' && Date.parse(value) ? formatDate(value) : formatValue(value)}
            </pre>
          </div>
        </div>
      );
