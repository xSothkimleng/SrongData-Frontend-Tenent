...
const DatasetDesignTab: React.FC<DatasetDesignTabProps> = ({ questionTypes, dataDesignForms, setDataDesignForms }) => {
  const lang = useLang(state => state.lang);
  useEffect(() => {
    if (dataDesignForms.length === 0) {
      setDataDesignForms([{ order: 1, label: '', type: '', data_type: '', is_required: true, options: [] }]);
    }
  ...
  const handleAddForm = useCallback(() => {
    setDataDesignForms(prevForms => [
      ...prevForms,
      { order: prevForms.length + 1, label: '', type: '', data_type: '', is_required: true, options: [] },
    ]);
  }, [setDataDesignForms]);
        ...
        <Grid container spacing={1}>
          {dataDesignForms.map((form, formIndex) => (
            <Grid item xs={12} key={form.order}>
              <AccordionContainer title={`${GetContext('question_no', lang)} ${form.order}`}>
                <Grid container spacing={2}>
                  ...
                  <Grid item xs={2}>
                    <TextField
                      ...
                      onChange={event => handleIsRequiredChange(formIndex, event.target.value === 'true')}
                      required>
                  ...
                  {form.type === 'single' && (
                    <Grid item xs={12}>
                      {form.options.map((option, optionIndex) => (
                        <Grid container spacing={2} alignItems='center' key={optionIndex} sx={{ marginBottom: '0.5rem' }}>
                          <Grid item>
                            <Radio disabled />
                          </Grid>
                          <Grid item xs>
                      ...
                      <Button onClick={() => handleAddOption(formIndex)}>{GetContext('add_option', lang)}</Button>
                    </Grid>
                  )}
                  {form.type === 'dropdown' && (
                    <Grid item xs={12}>
