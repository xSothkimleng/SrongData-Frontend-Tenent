...
const DatasetDesignTab: React.FC<DatasetDesignTabProps> = ({
  ...
  // New function to check if OTHER questions (not the current one) have skip logic
  const getSectionSkipLogicStatusForOtherQuestions = (sectionOrder: number, currentQuestionOrder: number) => {
    return dataDesignForms.some(
      form =>
        form.section?.order === sectionOrder &&
        form.order !== currentQuestionOrder && // Exclude current question
        form.skip_logics &&
                        ...
                        <Grid container spacing={2}>
                          ...
                          <Grid item xs={isSurveyInBothLanguages ? 2 : 4}>
                            <TextField
                              ...
                              value={form.type}
                              onChange={event => {
                                handleQuestionTypeChange(form.order, event.target.value);
                              }}
                              disabled={form.id !== undefined && isEdit && isEdit === true}
                              required>
                              ...
                              {form.options.map((optionValue, optionIndex) => {
                                ...
                                return (
                                  <Grid
                                    ...
                                    <Grid item xs={3}>
                                      {(hasSkipLogic ||
                                        !getSectionSkipLogicStatusForOtherQuestions(
                                          form.section?.order || 0,
                                          form.order, // Pass current question order to exclude it from check
                                        )) && (
                                        <Button
                                          variant='outlined'
