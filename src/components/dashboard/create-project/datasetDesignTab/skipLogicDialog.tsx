...
const SkipLogicDialog: React.FC<LogicDialogProps> = ({
  ...
  const [action, setAction] = useState<string>(
    currentSkipLogic?.action || "go_to",
  );
  const [targetSectionId, setTargetSectionId] = useState<number | null>(
    currentSkipLogic?.target || null,
  );
  ...
  const availableSections = sectionList.filter(
    (section) => section.order !== currentForm.section.order,
  );
        ...
        <Grid container spacing={2} sx={{ marginTop: 1 }}>
          <Grid item xs={12}>
            <Typography variant="subtitle1" gutterBottom>
              For question:{" "}
              <strong>{currentForm?.label.en || "Unknown Question"}</strong>
            </Typography>
          ...
          {action != "submit_form" && (
            <Grid item xs={12}>
              <FormControl fullWidth margin="normal">
                <InputLabel id="target-section-label">
                  Target Section
                </InputLabel>
                <Select
