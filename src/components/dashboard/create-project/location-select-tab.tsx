...
const LocationSelectionTabs = ({ isUpdate, isEdit }: IsUpdateProps) => {
  console.log("can edit location: ", isEdit);

  const lang = useLang((state) => state.lang);
  const locationTabLocal = isUpdate
    ? "locationTabValueEdit"
    : "locationTabValue";
  ...
  return (
    ...
    >
      ...
      {value == 0 && (
        <ProvinceSelectionTab isUpdate={isUpdate} isEdit={isEdit} />
      )}
      {value == 1 && (
        <DistrictSelectionTab isUpdate={isUpdate} isEdit={isEdit} />
      )}
      {value == 2 && (
        <CommuneSelectionTab isUpdate={isUpdate} isEdit={isEdit} />
      )}
      {value == 3 && (
        <VillageSelectionTab isUpdate={isUpdate} isEdit={isEdit} />
      )}
    </Box>
  );
};
