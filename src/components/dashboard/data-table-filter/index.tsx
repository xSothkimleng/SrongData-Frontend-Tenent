        ...
        return (
          <Box sx={{ display: "flex", alignItems: "center", height: "100%" }}>
            <Typography style={{ fontSize: "1rem" }}>
              {getLocaleValue(params.value, lang)}
            </Typography>
          </Box>
        );
      },
...
export default function ProjectSummaryTable() {
  ...
  const projectStatus = Array.isArray(rowData)
    ? Array.from(new Set(rowData.map((row) => row.status)))
    : [];
    ...
    <Grid container spacing={3}>
      <Grid container item xs={12} spacing={3}>
          ...
          <NameCounterCard
            ...
            count={
              Array.isArray(rowData)
                ? rowData.filter((row) => row.status === 1).length
                : 0
            }
        ...
        <Grid item xs={3}>
          <NameCounterCard
            ...
            count={
              Array.isArray(rowData)
                ? rowData.filter((row) => row.status === 0).length
                : 0
            }
        ...
        <Grid item xs={3}>
          <NameCounterCard
            ...
            count={
              Array.isArray(rowData)
                ? rowData.filter((row) => row.status === 2).length
                : 0
            }
      ...
      <Grid item xs={12}>
        <Box
          ...
          className="boxShadow-1 border-1 p-[0.6rem]"
        >
